...
public class PortfolioBacktestService {
	...
	public PortfolioBacktestResponseDTO calculatePortfolio(PortfolioBacktestRequestDTO request) {
		...
		try {
			...
			List<WeightedStockData> weightedStockDataList = new ArrayList<>();
			...
			for (PortfolioBacktestRequestItemDTO item : requestItems) {
				...
				if (item.isCustomStock()) {
					...
					stockMonthlyRor = stockMonthlyRorCache.getOrDefault(stock, Collections.emptyMap());
				...
				// 5. 포트폴리오 월별 수익률 계산을 위한 데이터 적재
				weightedStockDataList.add(new WeightedStockData(item.getWeight(), stockMonthlyRor));
			}

			Map<LocalDate, Float> portfolioMonthlyRor =
				calculatePortfolioMonthlyRor(weightedStockDataList, startMonth, endMonth);

			// 6. 포트폴리오 전체 누적(복리) 수익률 계산
			float totalRor = PortfolioCalculator.calculateCompoundRor(new TreeMap<>(portfolioMonthlyRor), startMonth,
				endMonth);

			// 7. 월별 누적 자산 금액 계산 (초기 자산: request.getAmount())
			Map<LocalDate, Long> monthlyAmount = calculateMonthlyAmounts(portfolioMonthlyRor, startMonth, endMonth,
				request.getAmount());
	...
	private Map<Stock, Map<LocalDate, Float>> calculateAllStockMonthlyRor(
		...
		for (Stock stock : stocks) {
			...
			result.put(stock, stockMonthlyRor);
		}

		return result;
	}

	private Map<LocalDate, Float> calculatePortfolioMonthlyRor(List<WeightedStockData> stockData,
		LocalDate startMonth,
		LocalDate endMonth) {
		Map<LocalDate, Float> portfolioMonthlyRor = new TreeMap<>();
		...
		while (!current.isAfter(endMonth)) {
			...
			for (WeightedStockData data : stockData) {
				Float stockMonthlyReturn = data.monthlyRor().get(current);
				if (stockMonthlyReturn != null) {
					availableWeight += data.weight();
					weightedReturnSum += data.weight() * stockMonthlyReturn;
				}
			}

			if (availableWeight == 0.0) {
				portfolioMonthlyRor.put(current, AppConstants.DEFAULT_MONTHLY_ROR);
			} else {
				float adjustedReturn = (float)(weightedReturnSum / availableWeight);
				portfolioMonthlyRor.put(current, adjustedReturn);
			}

			current = current.plusMonths(1);
		}

		return portfolioMonthlyRor;
	}
	...
	private record WeightedStockData(float weight, Map<LocalDate, Float> monthlyRor) {
	}
}
